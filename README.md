...
とすると、割り込み処理が終わらなかった場合に、次のVSYNC割り込みをスキップしてくれます。現在はこれがデフォルトの動作なので、明示的に指定する悲追用はありません。

一方、この機能がうまく働かないゲームもあるようです。割り込み処理が終わったことを検出で記事雨に、必要以上にスキップしてしまうようなケースです。対して重そうに見えないのに、やたらと動作が遅い場合は、以下のようにして自動フレームレート調整を無効にしてみてください。
