...
/*
	...
	* D1Xモード / D2Xモード / D4Xモード
	...
	画面が「256ドットモード、スプライトが16ドットモード、拡大なし」の場合とそれ以外の場合で
	...
	その代わり、後述する「特殊なケース」では、スプライトパターン定義とスプライト番号の関係を固定せずに、
	必要なパターンを再利用するようにしています。
	...
	X68000のPCGは128個定義できるので多めに持つことも可能ですが、すでに色合成済みのものがあるか
	どうかを調べる際にリニアサーチを行うため、あまり多く持ちすぎるとかえって遅くなる
	可能性があり、32個程度が適切だと考えて割り切っています。
...
void refresh_sprite_256_mode2(ms_vdp_t* vdp) {
	...
	uint32_t alt_color_zero = (vdp->r08 % 0x20) ? 0 : vdp->alt_color_zero;
