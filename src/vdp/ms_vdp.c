...
void ms_vdp_update_visibility(ms_vdp_t* vdp) {
    ...
    uint16_t r02 = 0;
    // グラフィックとテキストパレット#0を半透明合成
    // スプライト+BG面は常に表示(スプライトの表示/非表示は個別のスプライトのON/OFFで行っている)
    r02 |= 0b01000000;
    if (vdp->tx_active) {
        // r02 |= 0b0101110000100000;  // 半透明はうまくいかないので(研鑽が足りない)パレットを半分にして対応
