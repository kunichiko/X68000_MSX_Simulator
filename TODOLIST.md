...
### メモリマップドライバ (2024/8/26達成済)

MSXには通常のROM、メインメモリ、メガロムなど、様々な種類のデバイスがメモリ上に配置されますが、それらを柔軟に管理するためにドライバ機構を導入します。
...
* LEDMODを都度呼ぶのではなく、VSYNC期間にまとめて反映させる
	* 正直 0.2秒に一回くらい反映させればいいかなと思っています
* ホスト側のVSYNC処理のタイミングの見直し
	* 現状は、CPUにVSYNC割り込みをかけたあとに一定サイクル(hostrate値で決定)実行後に、ホスト側の処理を呼び出しています
	* スプライト消えへの対策を行なっている多くのゲームはVSYNC割り込み中にスプライトアトリビュートテーブルを書き換えて、スプライトの優先順位をごっそり入れ替えたりしています
	* できればその辺りの処理が終わったあたりを見計らってホスト側の処理を呼び出し、画面描画が始まる前くらいにスプライトの表示/非表示状態などの辻褄合わせを行いたいです
	* 一方、ホスト側の処理が早く呼ばれすぎると割り込み処理ルーチンが終わる前に始まってしまい、うまく反映できなくなります
	* このあたりを hostrateで調整するのはちょっと難しいので、ラスタ割り込みを使って、画面表示期間に入る直前にホスト側の処理を呼び出すようにするのがいいかなと思っています
